...
for i=1:length(SNR)
      ...
      %end
          ...
          %接收端初始时就要进行STO估计,com_delay公共时延为0；
          %[STO_est,mag]=STO_by_difference(y_GI(1:Nfft+Ng),Nfft,Ng,0);
          %STO估计中是按照一整帧进行定时同步的，如果需要用STO_by_xx，则需要对各个OFDM符号组合在一起
