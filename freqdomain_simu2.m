      ...
      %%   形成一个OFDM符号 
       ...
       %  x_GI = [x(Nfft-Ng+1:Nfft) x];                  % Add CP|加循环前缀
         %% 信道仿真
            ...
            H_orignal= fft(h,Nfft);  % True channel and its time-domain length|实际信道和它的长度
           ...
           Y_rece=H_ch.*X_symbol;
            y=ifft(Y_rece,Nfft);
          sigPow = mean(y.*conj(y));        
          noisemag=sqrt(  (sigPow*10^(-SNR(i)/10))  /2 );
        
         % Y_data_pilot=Y_rece+noisemag*(randn(size(Y_rece))+j*randn(size(Y_rece)));
          Y_data_pilot=awgn(Y_rece,SNR(i),'measured');
     ...
     %% 不同估计方法比较
              for q=1:3
                  ...
                  elseif q==2
                      ...
                      H_est3 = MMSE_CE( Y_data_pilot,Xp,pilot_loc,Nfft,Nps,h2,10*log10(1/noisePow)); method='MMSE'; % MMSE estimation
                  end
                 ...
                 if(q==3)
                       h_est3=ifft(H_est3);
%                         h_DFT=zeros(1,Lch);
%                       h_DFT(Delay+1) = h_est3(Delay+1); 
                       h_DFT = h_est3(1:Lch); 
                 else
                    % h_DFT=h_est(1:Lch);
                     %去噪
%                       h_DFT=zeros(1,Lch);
%                       h_DFT(Delay+1) = h_est(Delay+1); 
                      h_DFT=h_est(1:Lch);
                 end
                 ...
                 %H_DFT=H_LSdft;
;
                    MSE(q) = MSE(q) + norm(ifft(H_orignal)-ifft(H_est),'fro');
                    MSE(q+3) = MSE(q+3) + norm(ifft(H_orignal)-ifft(H_LSdft),'fro');
                    MSE(q+6) = MSE(q+6) + norm(ifft(H_orignal)-ifft(H_DFT),'fro');
              end
 ...
 %%%%%信道估计归一化均方误差
    for count=1:length(SNR)
        NMSE(count,:)=20*log10(   MSEs(count,:)./(  norm(ifft(H_orignal),'fro') ) );
    end  %计算归一化均方误差
       

     figure(3)
    plot(SNR,NMSE(:,1),'g-s');hold on
    plot(SNR,NMSE(:,2),'c-s');hold on
    plot(SNR,NMSE(:,9),'r--*');hold on
    plot(SNR,NMSE(:,3),'r--*');hold on
    plot(SNR,NMSE(:,4), 'b-d');hold on
    plot(SNR,NMSE(:,7),'b-d');hold on
   % plot(SNR,NMSE(:,8),'b--s');hold on 
    legend('LS linear interpolate','LS spline interpolate ','MMSE with denoise ','only MMSE','LS DFT interpolate','LS DFT interpolate and denoise');
